...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo mkdir -p /opt/defguard/init",
      "sudo mv /tmp/docker-compose.template.yaml /opt/defguard/init/docker-compose.template.yaml",
      "sudo mv /tmp/lib.sh /opt/defguard/init/lib.sh",
      "sudo mv /tmp/generate-env.sh /opt/defguard/init/generate-env.sh",
      "sudo mv /tmp/generate-compose.sh /opt/defguard/init/generate-compose.sh",
      "sudo chmod +x /opt/defguard/init/generate-env.sh /opt/defguard/init/generate-compose.sh",
      "echo 'DEFGUARD_CORE_TAG=${var.core_tag}' | sudo tee /opt/defguard/init/.image-tags > /dev/null",
      "echo 'DEFGUARD_PROXY_TAG=${var.proxy_tag}' | sudo tee -a /opt/defguard/init/.image-tags > /dev/null",
      "echo 'DEFGUARD_GATEWAY_TAG=${var.gateway_tag}' | sudo tee -a /opt/defguard/init/.image-tags > /dev/null",
      ...
      "sudo chown -R root:root /opt/defguard",
      "sudo chmod -R go-w /opt/defguard/init",
