...
# `docker compose up -d` with no COMPOSE_PROFILES set. Requesting JSON output
# and stripping the key with jq avoids that; JSON is valid YAML, so the result
# is written directly as the final docker-compose.yml.
{
  echo "# Generated by defguard OVA first-boot init on $(date -u +%FT%TZ)."
  echo "# Selected profiles: ${_profiles[*]}"
  docker compose -f "$TEMPLATE" --project-directory "$STACK_DIR" --env-file "$STACK_DIR/.env" "${_profile_args[@]}" \
    config --format json \
